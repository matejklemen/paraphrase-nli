...
from src.data.bitext import WMT14TransformersDataset, CCMatrixTransformersDataset
...
parser.add_argument("--pretrained_name_or_path", type=str,
                    default="ccmatrix-nl-sl-bitext-xlm-roberta-base-2e-5-maxlength112")
parser.add_argument("--model_type", type=str, default="xlm-roberta",
                    choices=["bert", "roberta", "xlm-roberta"])
parser.add_argument("--max_seq_len", type=int, default=112)
...
parser.add_argument("--train_path", type=str, help="Path to the training set of CCMatrix (tsv)", default=None)
parser.add_argument("--dev_path", type=str, help="Path to the validation set of CCMatrix (tsv)", default=None)
parser.add_argument("--test_path", type=str, help="Path to the test set of CCMatrix (tsv)", default=None)
...
def get_predictions(model, test_set, pred_strategy="argmax", num_mcd_rounds=0, thresh=None):
    ...
    if pred_strategy == "argmax":
        ...
        valid_preds = torch.gt(mean_proba[:, test_set.label2idx["not_translation"]], thresh)

        preds[valid_preds] = test_set.label2idx["not_translation"]
    else:
...
if __name__ == "__main__":
    ...
    for dataset_name, data_path in [("train", args.train_path), ("dev", args.dev_path), ("test", args.test_path)]:
        ...
        dataset = CCMatrixTransformersDataset(data_path, tokenizer=tokenizer,
        ...
        if args.l2r_strategy == "ground_truth":
            ...
            para_mask = torch.eq(dataset.labels, dataset.label2idx["translation"])
            ...
            para_mask = torch.eq(res["pred_label"], dataset.label2idx["translation"])
            l2r_labels = res["pred_label"]
            mean_probas = res["mean_proba"][:, dataset.label2idx["translation"]]
            sd_probas = res["sd_proba"][:, dataset.label2idx["translation"]]
        ...
        logging.info(f"Left-to-right: {len(para_inds)} translations found!")
        ...
        # Note that for right-to-left we are interested in P(y=not_translation) (filtering!)
        r2l_preds["label"] = list(map(lambda i: dataset.idx2label.get(i, "other"), reverse_res["pred_label"].tolist()))
        r2l_preds["mean_proba"] = reverse_res["mean_proba"][:, dataset.label2idx["not_translation"]].tolist()
        r2l_preds["sd_proba"] = reverse_res["sd_proba"][:, dataset.label2idx["not_translation"]].tolist()

        rev_nonpara_mask = torch.eq(reverse_res["pred_label"], dataset.label2idx["not_translation"])
        ...
        logging.info(f"{len(rev_nonpara_inds)} non-translations found!")
        ...
        logging.info(f"Writing false translations for dataset '{dataset_name}' ({len(false_paras['sequence1'])} examples)")
        model_metrics[f"false_translations_{dataset_name}"] = len(false_paras['sequence1'])
        pd.DataFrame(false_paras).to_csv(os.path.join(args.experiment_dir, f"{dataset_name}_false_translations.csv"),
                                         sep=",", index=False, quoting=csv.QUOTE_ALL)
    ...
    logging.info(f"Writing combined false translations ({len(all_false_paras['sequence1'])} examples)")
    pd.DataFrame(all_false_paras).to_csv(os.path.join(args.experiment_dir, f"all_false_translations.csv"),
                                         sep=",", index=False, quoting=csv.QUOTE_ALL)
