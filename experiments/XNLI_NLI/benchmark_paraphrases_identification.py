...
parser.add_argument("--mnli_para_dir", type=str, default="mnli-roberta-base-argmax")
parser.add_argument("--xnli_para_dir", type=str, default="xnli-all-xlm-roberta-base-argmax")
...
if __name__ == "__main__":
	...
	df = pd.read_csv(os.path.join(args.mnli_para_dir, "train_paraphrases.csv"))
	...
	dev_df = pd.concat((
		pd.read_csv(os.path.join(args.mnli_para_dir, "validation_matched_paraphrases.csv")),
		pd.read_csv(os.path.join(args.mnli_para_dir, "validation_mismatched_paraphrases.csv"))
	))
	# Basic data cleaning - remove NAs (?), duplicate pairs, pairs with one sequence very short
	dev_df = dev_df.loc[mask_not_na(dev_df["sequence1"], dev_df["sequence2"])]
	dev_df = dev_df.iloc[inds_unique(dev_df["sequence1"], dev_df["sequence2"])]
	dev_df = dev_df.loc[mask_long_enough(dev_df["sequence1"], dev_df["sequence2"])]

	# Balance dataset by undersampling majority class
	uniq_classes, uniq_counts = np.unique(dev_df["label"].values, return_counts=True)
	minority, majority = np.argmin(uniq_counts), np.argmax(uniq_counts)
	minority_count, majority_count = uniq_counts[minority], uniq_counts[majority]
	dev_df = pd.concat((
		dev_df.loc[dev_df["label"] == minority].reset_index(drop=True),
		dev_df.loc[dev_df["label"] == majority].sample(n=minority_count).reset_index(drop=True)
	))

	test_df = pd.read_csv(os.path.join(args.xnli_para_dir, "all_test_paraphrases_prepr.csv"))
