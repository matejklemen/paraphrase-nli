...
from src.data.bitext import RSDO4TransformersDataset
...
parser.add_argument("--train_path", type=str, help="Path to training set of RSDO4 (tsv)",
                    default="train.tsv")
parser.add_argument("--dev_path", type=str, help="Path to dev set of RSDO4 (tsv)",
                    default="dev.tsv")
parser.add_argument("--test_path", type=str, help="Path to test set of RSDO4 (tsv)",
                    default="test.tsv")
...
parser.add_argument("--validate_every_n_examples", type=int, default=20000)
...
if __name__ == "__main__":
    ...
    train_set = RSDO4TransformersDataset(args.train_path, tokenizer=tokenizer,
                                         max_length=args.max_seq_len, return_tensors="pt",
                                         reverse_order=args.reverse_order,
                                         nrows=args.nrows)
    dev_set = RSDO4TransformersDataset(args.dev_path, tokenizer=tokenizer,
                                       max_length=args.max_seq_len, return_tensors="pt",
                                       reverse_order=args.reverse_order)
    test_set = RSDO4TransformersDataset(args.test_path, tokenizer=tokenizer,
                                        max_length=args.max_seq_len, return_tensors="pt",
                                        reverse_order=args.reverse_order)
