...
from src.data.paraphrase import WSCTransformersDataset
...
parser.add_argument("--train_path", type=str, help="Path to training set of preprocessed WSC (tsv)",
                    default="/home/matej/Documents/paraphrase-nli/experiments/WSC_coref/id_wsc_train.tsv")
parser.add_argument("--dev_path", type=str, help="Path to dev set of preprocessed WSC (tsv)",
                    default="/home/matej/Documents/paraphrase-nli/experiments/WSC_coref/id_wsc_dev.tsv")
parser.add_argument("--test_path", type=str, help="Path to test set of preprocessed WSC (tsv)",
                    default="/home/matej/Documents/paraphrase-nli/experiments/WSC_coref/id_wsc_test.tsv")

parser.add_argument("--num_epochs", type=int, default=10)
parser.add_argument("--max_seq_len", type=int, default=166)
...
if __name__ == "__main__":
    ...
    tokenizer.add_special_tokens({
        "additional_special_tokens": ["<ENTITY1>", "</ENTITY1>", "<ENTITY2>", "</ENTITY2>"]
    })
    tokenizer.save_pretrained(args.experiment_dir)

    train_set = WSCTransformersDataset(args.train_path, tokenizer=tokenizer,
                                       max_length=args.max_seq_len, return_tensors="pt",
                                       reverse_order=args.reverse_order)
    dev_set = WSCTransformersDataset(args.dev_path, tokenizer=tokenizer,
                                     max_length=args.max_seq_len, return_tensors="pt",
                                     reverse_order=args.reverse_order)
    test_set = WSCTransformersDataset(args.test_path, tokenizer=tokenizer,
                                      max_length=args.max_seq_len, return_tensors="pt",
                                      reverse_order=args.reverse_order)
    ...
    for curr_thresh in ["argmax", 0.75, 0.9]:
        if curr_thresh == "argmax":
            ...
            curr_pred = np.logical_not(
                test_res["pred_proba"][:, test_set.label2idx["not_coreferent"]].numpy() > curr_thresh
            ).astype(np.int32)
